{{-- admin/cedis/partials/cedis_rows.blade.php --}}
@if (isset($cedis) && $cedis->count() > 0)
    @foreach ($cedis as $cedi)
        <tr>
            {{-- Columna para Nombre --}}
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900">{{ $cedi->nombre }}</div>
            </td>

            {{-- Columna para Dirección --}}
            <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm text-gray-500">{{ $cedi->direccion ?? 'Sin dirección' }}</div>
            </td>

            {{-- Columna para Región --}}
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ $cedi->region->nombre ?? 'N/A' }}
            </td>

            {{-- Columna para Responsable --}}
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ $cedi->responsable ?? 'N/A' }}
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <div class="flex items-center space-x-3">
                    ...
                    <form action="{{ route('admin.cedis.toggle-status', $cedi->id) }}" method="POST"
                        class="inline-block">
                        @csrf
                        @method('PATCH')
